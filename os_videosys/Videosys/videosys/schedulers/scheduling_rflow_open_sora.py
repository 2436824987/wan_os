...
class RFLOW:
    ...
    def sample(
        ...
        if ea_timesteps is not None:
            ea_timesteps = [torch.tensor(t) for t in ea_timesteps] # Add ea_timesteps
            ea_timesteps = [t.unsqueeze(0).cuda() for t in ea_timesteps]
        
        # construct full timesteps
        ...
        if ea_timesteps:
            ea_timesteps_ = [int(t.to(dtype).item()) for t in ea_timesteps] 
        ...
        for i, t in progress_wrap(list(enumerate(timesteps))):
            ...
            t_ = int(t.to(dtype).item())

            if token_timesteps is None:
                token_timesteps = [1.0] * len(timesteps)
            token_timesteps[0] = 1.0

            if token_timesteps is None or latest_pred_output is None or token_timesteps[i] > 0:
                percentage = token_timesteps[i]
                # classifier-free guidance
                z_in = torch.cat([z, z], 0)
                t = torch.cat([t, t], 0)

                # pred = model(z_in, t, **model_args).chunk(2, dim=1)[0]
                output = model(z_in, t, timesteps, percentage=percentage, **model_args)
                latest_pred_output = output
            else:
    ...
    def construct_full_timesteps(
        ...
        return [tensor.item() for tensor in timesteps]
