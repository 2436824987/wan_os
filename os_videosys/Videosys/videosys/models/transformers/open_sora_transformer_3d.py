...
class STDiT3Block(nn.Module):
    ...
    def forward(
        ...
        if enable_pab() and broadcast_attn:
            ...
            # attention
            if percentage is None or percentage == 1:
                if self.temporal:
                    if self.parallel_manager.sp_size > 1:
                        x_m, S, T = self.dynamic_switch(x_m, S, T, to_spatial_shard=True)
                    x_m = rearrange(x_m, "B (T S) C -> (B S) T C", T=T, S=S)
                    x_m = self.attn(x_m)
                    self.diff = torch.abs(x_m - self.hs).sum(dim=[0,2])
                    self.hs = x_m
                ...
                if self.temporal:
                    ...
                    print("T",T)
                    print("S",S)
                    num = max(1, int(T * percentage))
                    if self.block_idx == 0:
                        indices = torch.topk(self.diff, num, dim=0).indices
                        indices = torch.sort(indices)[0]
                    print(indices)
                    x_m = self.attn(x_m, flag=True, indices=indices)
                    diff = torch.abs(x_m - self.hs[:, indices, :]).sum(dim=[0, 2])
...
class STDiT3(PreTrainedModel):
    ...
    def forward(
        self, x, timestep, all_timesteps, y, mask=None, x_mask=None, fps=None, height=None, width=None, percentage: float=None, **kwargs
    ):
        # === Split batch ===
